...
void Face::draw(DrawContext *ctx) {
  ...
  // Borde inferior de color (mood flash)
  uint16_t borderColor = ctx->getColorPalette()->get(COLOR_BORDER);
  uint16_t bgColor     = ctx->getColorPalette()->get(COLOR_BACKGROUND);
  if (borderColor != bgColor) {
    int16_t w = boundingRect->getWidth();
    int16_t h2 = boundingRect->getHeight();
    sprite->fillRect(0, h2 - 5, w, 5, borderColor);
  }
