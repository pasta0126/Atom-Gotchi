...
class Effect final : public Drawable {
 ...
 public:
  ...
  void draw(M5Canvas *spi, BoundingRect rect, DrawContext *ctx) override {
    ...
    // Iconos manga en color de acento del mood (COLOR_SECONDARY); fallback a blanco
    uint16_t iconColor    = ctx->getColorDepth() == 1 ? 1 : ctx->getColorPalette()->get(COLOR_SECONDARY);
    if (iconColor == bgColor) iconColor = primaryColor;
    float offset = ctx->getBreath();
    Expression exp = ctx->getExpression();
    // Positions scaled to 128×128 (original was 320×240, factor x=0.40 y=0.53)
    switch (exp) {
      case Expression::Doubt:
        drawSweatMark(spi, 116, 59, 3, iconColor, -offset);
        break;
      case Expression::Angry:
        drawAngerMark(spi, 112, 27, 5, iconColor, bgColor, offset);
        break;
      case Expression::Happy:
        drawHeartMark(spi, 112, 27, 5, iconColor, offset);
        break;
      case Expression::Sad:
        drawChillMark(spi, 108, 4, 12, iconColor, offset);
        break;
      case Expression::Sleepy:
        drawBubbleMark(spi, 116, 21, 4, iconColor, offset);
        drawBubbleMark(spi, 108, 28, 2, iconColor, -offset);
        break;
      default:
        // noop
        break;
